...
def get_nodes_shortcuts(_menu_nodes=nuke.menu("Nodes"),
    ...
    # any hotkeys.
    if not qtHelper.check_action_is_checked(config_key="show_shortcuts_in_tab_menu"):
        return {}
    ...
    for item in _menu_nodes.items():
        ...
        if item.__class__.__name__ == "Menu":
            ...
            sub_hotkeys = get_nodes_shortcuts(_menu_nodes=item,
...
class EventFilter(QObject):
    def __init__(self, node_graph_widget=None, show_hotkeys: bool = True):
    ...
    def change_widget_behaviour(self):
        ...
        if isinstance(widget, QLineEdit):
            ...
            proxy_model = CustomProxyModel(nodes_shortcuts=get_nodes_shortcuts())
...
def start():
    ...
    _filter = EventFilter(node_graph_widget)

    if qtHelper.check_action_is_checked(config_key="show_shortcuts_in_tab_menu"):
